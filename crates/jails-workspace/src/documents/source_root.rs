...
pub(crate) fn ensure_maven_source_roots(
    ...
    // while it is the last thing in there -- and it was, until a build feature
    // started landing beside it. After that every plan wanted to move one past
    // the other, so `jails model check --frozen` reported a pending operation
    // on a project that had just been synchronised, and the pom churned by a
    // block on every run.
    //
