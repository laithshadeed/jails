...
pub(crate) fn reconcile_properties(
    ...
    // **A `# jails:<name>` block is jails' own, however old.** The engine this
    // replaces wrapped each capability's properties in one; the compiler
    // claims a key at a time and has no markers. A project generated by that
    // engine therefore arrives with keys jails wrote inside markers jails
    // wrote -- and reading them as the reader's refused `add db` on it
    // permanently, with a fix line telling the reader to delete a line they
    // had never written. The block is adopted and its markers dissolve.
