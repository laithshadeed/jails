...
pub(crate) struct Written<'a> {
    ...
    /// The parent whose key this write resolves, when the caller sends a
    /// component of the parent instead of the key itself.
...
pub(crate) fn usecase_files(
    ...
    let resolution = via
        .map(|parent| via::resolve(slice, name, target, fields, parent))
        .transpose()?;
    for field in fields {
        // The lookup names a component of the *parent*, deliberately: it is
        // what the caller sends instead of the key. Checking it against the
        // target would refuse the one field `--via` exists for.
        if resolution
            .as_ref()
            .is_some_and(|resolution| resolution.lookup.name == field.name)
        {
            continue;
        }
        let Some(target_field) = target_fields
            ...
            return Err(format!(
                ...
                match via {
                    Some(parent) => format!(
                        " -- and `--via {parent}` already names `{}` as the one it looks the \
                         parent up by",
                        resolution
                            .as_ref()
                            .map(|resolution| resolution.lookup.name.as_str())
                            .unwrap_or_default()
                    ),
                    None => String::new(),
                }
    ...
    for field in target_fields {
        // The reference is selected from the parent's own row, so there is
        // nothing to infer for it and no expression to render. A placeholder
        // here would be a value the insert then has to remember to ignore.
        if resolution
            .as_ref()
            .is_some_and(|resolution| resolution.join.child_component == field.name)
        {
    ...
    let mut artifacts = vec![
        ...
        Artifact {
            ...
            // Optional when the key is resolved: "no such parent" is an
            // expected outcome, and an expected outcome is a return value.
            contents: usecase_port_java(slice, name, target, resolution.is_some()),
        },
    ];
    if let Some(resolution) = &resolution {
        let adapters = slice.owned(Layer::Adapters);
        artifacts.push(Artifact {
            kind: "key-resolving use case",
            path: crate::generate::main_dir(slice.project().root(), &adapters)
                .join(format!("Resolving{name}UseCase.java")),
            contents: via::resolving_usecase_java(
                slice,
                name,
                target,
                &defaults,
                (resolution, &target_columns),
            ),
        });
        artifacts.push(Artifact {
            kind: "key-resolving integration test",
            path: crate::generate::test_dir(slice.project().root(), &adapters)
                .join(format!("Resolving{name}UseCaseIT.java")),
            contents: via::resolving_usecase_it_java(slice, name, fields, resolution),
        });
    ...
    artifacts.extend([
        Artifact {
            ...
            contents: usecase_controller_java(
                slice,
                target,
                name,
                fields,
                (endpoint, resolution.is_some()),
            ),
        },
        Artifact {
            kind: "usecase controller test",
            path: test_web.join(format!("{name}ControllerTest.java")),
            contents: usecase_controller_test_java(
                slice,
                name,
                &resolved,
                fields,
