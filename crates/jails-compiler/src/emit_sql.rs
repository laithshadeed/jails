...
pub(crate) fn derive(
    ...
    if accepted.is_none() && !snapshot.migration_history.records.is_empty() {
        return Err(Diagnostic::new(
            ...
            "$.capabilities.db",
            "existing migrations have no accepted-schema lock",
            "import the existing schema before enabling the `db` capability",
        ));
    }
            ...
            match policies.retirements.get(old.id.as_str()) {
                ...
                _ => {
                    return Err(Diagnostic::new(
                        ...
                        format!("use schema retirement before removing `{}`", old.label),
                    ));
                }
            }
        };
        ...
        if old.names.sql_table != current.names.sql_table {
            ...
            if policies.table_renames.get(old.id.as_str()) != Some(&current.names.sql_table) {
                return Err(Diagnostic::new(
                    ...
                    format!(
                        "`jails rename entity {} <NewName> --strategy single-cutover`, or keep the table with `--strategy preserve-table`",
                        old.names.java_type
                    ),
                ));
            }
            ...
            let Some(current_field) = current.field(&old_field.id) else {
                let Some(confirmed) = policies.removals.get(old_field.id.as_str()) else {
                    return Err(Diagnostic::new(
                        ...
                        "use field drop with the column name confirmed",
                    ));
                };
    ...
    if next.project.dialect != "postgresql" {
        return Err(Diagnostic::new(
            ...
            "$.project.dialect",
            format!(
                "schema evolution does not lower dialect `{}` yet",
    ...
    let mut bytes = b"-- Generated by jails from the accepted model.\n".to_vec();
