...
pub(crate) fn lower_and_emit(
    ...
    for entity in model.entities.values().filter(|entity| entity.active) {
        for facet in &entity.facets {
            ...
            // single-file arm below emitted only the port -- an interface with
            // no implementation, no route and no caller -- so a canonical
            // scaffold served nothing.
            if *facet == Facet::Http {
...
fn operation_record_shape(
    ...
    let components = parameters
        ...
        .map(|parameter| {
            ...
            // **The field's own Java projection, not the label.** A parameter
            // names a field in the model's alphabet -- `user_id` -- and
            // rendering that straight into Java gave `record Input(long
            // user_id)` beside `Message.userId()`, so the generated request
            // body bound `userId` to nothing and Jackson mapped `null` into a
            // primitive. The flat `record_shape` path has always used
            // `java_member`; this one is the copy that drifted.
            Ok(RecordComponent {
                name: parameter_member(model, parameter),
                ty,
...
pub(crate) fn render(
    ...
    // **An import of the destination package is dropped here, once.** A
    // sample for a model-declared type carries that type's import with it, the
    // way a builtin's does -- otherwise a constructor call reaching a project
    // enum compiles only where the emitter happened to render into the same
    // package as the enum, which is how the operation controller test named
    // `MessageDirection.TO_USER` with nothing importing it. Carrying the
    // import always and dropping the redundant one here is the version with a
    // single rule; making each caller decide is the version that gets it wrong
    // in the one place nobody generates by hand.
    //
    // Static imports are never dropped: `import static Foo.bar` is a different
    // statement and is needed from inside the package too.
    let kept = imports
        .iter()
        .filter(|import| {
            import.starts_with("static ")
                || import
                    .rsplit_once('.')
