...
const HANDLER: &str = include_str!("../../../../templates/spring/handler_java.java");
const TEST: &str = include_str!("../../../../templates/spring/handler_test_java.java");
const API_ERROR: &str = include_str!("../../../../templates/spring/api_error_java.java");
const API_ERROR_TEST: &str = include_str!("../../../../templates/spring/api_error_test_java.java");
...
pub(super) fn files(model: &AppModel, component: &Component) -> Result<Vec<Emitted>, CompileError> {
    ...
    // Empty when the two packages coincide: importing a sibling is a compile
    // error, which is what `--package ''` produces.
    let extra = if api == domain {
        String::new()
    } else {
        format!("import {domain}.ApiError;\n")
    };
...
pub(super) fn envelope(model: &AppModel) -> Result<Vec<Emitted>, CompileError> {
    let owners = model
        .components
        .values()
