...
`TestExecutionPlanV1`, `TestReportV1`). `docs/60-abstraction.md` S60.6 makes
that one vocabulary with the daemon's v2 protocol, which frames `serde`
values; when it lands, delete the codec, the derive crate, the board's
*types whose wire format is hand-written* and *codec halves outside `impl
Codec`* rows, and the `extern crate self` line in `jails-support`.
