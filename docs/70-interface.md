...
**Nothing is left on the prototype list.** All six came off it without
one, for the same reason: every part of each already existed. Bare `jails`
as status (I71.17) is facts other commands compute, `jails undo` (I71.15)
is the last plan read backwards, the MCP server (I71.20) is the clap tree
in a JSON-RPC envelope, the language server (I71.19) is `editor`'s own
answers in the envelope editors already speak, the manifest folded into
the model (I71.21) is a file copy and one `sync`, and the second `jails
...
**I71.21 — the manifest is the model.** *Landed, and the second source is
gone rather than deprecated.* `jails new <name> --model <file.jdl>` copies
the file in as the project's own `.jails/model.jdl` and compiles it, which
is a copy and one `sync`. `--app` is a visible alias so a script that
passes it keeps working; a `.toml` handed to either is refused by name, and
so is a project still carrying `.jails/app.toml`, for the same reason
`.jails/model.toml` is: a file declaring capabilities that nothing applies
is worse than no file.

**The project's identity wins over the file's.** A model file declares an
application and `new` just created a different one, so `pkg`, `java`,
`platform` and `build` come from the command line -- the reader typed those
flags a second ago and wrote the file last year -- and `storage` and every
other declaration are kept verbatim. That is what makes one model file
start any project.

**The six examples carry a `model.jdl` and nothing else**, generated by
applying each manifest and keeping what it produced, so the corpus the
specification lacked is real output rather than hand-written. Measured, per
example: the Java, resources and build files are byte-identical between the
manifest's replay and one compile of the model it produced. The only
difference is the migrations, which are irreproducible by contract -- a
replay writes one per row, a compile writes one for the whole model -- and
the combined file orders tables before foreign keys and applies cleanly to
a scratch database.

**What went with it.** `src/app.rs` and its manifest parser, `jails app
init|plan|apply`, the closed manifest schema, and the batching machinery
that existed only for the replay: `Invocation::batching`,
`defer_unchanged`, `batch_report`, `file_row`, `file_note` and the
process-wide owed-formatter. One model is one plan, so the formatter runs
once because there is one execution rather than because a flag deferred it.
Two board rows fell: `root: &Path` parameters 77 to 76, and refusals with
no `fix:` line 80 to 69 -- eleven of those were about the manifest's own
closed schema, and a file that no longer exists needs no next step.

*Deviation:* the item said the examples would give the specification "the
corpus it lacks", and they do, but the generated models are written the way
the compiler renders them rather than the way the specification's examples
are hand-laid-out. They are a corpus of real declarations, not a style
guide.
...
**I71.2 — a manifest replay is one capture.** *Landed, then made moot by
I71.21.* The item proposed linking every row into one edited source. What
landed first kept a row's own pipeline for a row that changed something
and skipped it for a row that did not, taking the crawler's twelve rows
from 255 ms to 30 ms; what landed second deleted the manifest, and with it
the replay, the skip and `Invocation::defer_unchanged`. A model is one
capture, one compile and one plan whatever it declares, which is the item
as originally written rather than the route around it, and a converged
project is recognised as one before a pipeline is built at all.
`a_model_is_one_pipeline_whatever_it_declares` asserts the shape rather
than the milliseconds: three declarations produce one plan digest, and the
