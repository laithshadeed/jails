...
**First, the structural change (§1):** generated Java lives in `src/` like
any other Java, and `.jails/` holds only what jails needs to generate
again. Today the application lives under the dotdir, deleting it compiles
an empty application in two seconds without a word, and five tool-side
scanners, the test selector among them, cannot see generated code at all.
S60.7 in `docs/60-abstraction.md` names the mechanism; §1 measures why it
...
| project | `model.jdl` | `compiler.lock.json` | `.jails/generated` | files under `src/` |
|---|---:|---:|---:|---:|
| fresh `new` | 386 B | 6 KB | -- | 5 |
| one scaffold | 814 B | 468 KB | 23 files, 31 KB | 5 |
| twelve capabilities | 899 B | 1.9 MB | 53 files, 119 KB | 7 |
| web-crawler example | 2.6 KB | 3.7 MB | 99 files, 239 KB | 12 |
| a hundred scaffolds | 17.7 KB | 21.4 MB | 1,417 files, 1.4 MB | 5 |

`ls` at the root of the one-entity project prints `AGENTS.md mise.toml
pom.xml src target`. `rg 'class NoteService'` finds nothing (exit 1) and
finds it with `--hidden`. The pom carries a `build-helper-maven-plugin`
block with three executions so the build, and the IDE through it, can
find the code. `.jails/` also holds an empty `apply.lock`, `run/` (a
classpath cache, an affected-index file, the daemon's socket), `app.toml`
on manifest projects, and the names of three legacy formats the code
refuses by name.

**Delete `.jails`, build.** `rm -rf .jails target && mvn test-compile`
returns 0 in 2 s. The build-helper block adds a source root that no longer
exists and Maven does not mind; the one remaining source is the
application class. Every domain type is gone and no line says so.

**Relocate by hand, delete `.jails`.** Copy `.jails/generated/main/java`
over `src/main/java`, the test tree and test resources likewise,
`requests/*.http` to `src/test/http`, remove the 1,700-byte build-helper
block: 28 files under `src/`, and `mvn test` is green in 8 s with the same
tests. The generated Java is self-contained. Two generated files are not:
`ArchitectureTest.java` reads `.jails/architecture.toml` for its policy
and `archunit.properties` points ArchUnit's freeze store at
...
Twenty of the 23 generated Java files carry the `// Generated by jails
from art_…` header; `ArchitectureTest.java` does not.

**What the move touches.** The string `.jails/generated` appears at 124
sites in 29 production files, one of them `MANAGED_ROOT` in the compiler;
the rest re-derive it.

**Five scanners read `src/` alone**, so each answers a question about a
scaffolded project as if the scaffold were not there:

| scanner | file | roots it reads | what a reader sees |
|---|---|---|---|
| warm test universe | `run/isolation.rs` | `src/test/java` | `jails testd NoteTest` and `test --engine warm` refuse every generated test (*no attributable test source*); a hand-written test under `src/test/java` runs in about 100 ms by its bare name, as README promises |
| staleness fingerprint | `run/fingerprint.rs` | `src/{main,test}/{java,resources}`, build files | `test --fast` widens every generated test to Maven, so the blind spot is hidden until the row above is fixed |
| affected index | `affected.rs` | `src/main/java`, `src/test/java` | a comment appended to the generated `Note.java`, then `test --affected`: *no affected tests in epoch 1*, 27 ms, exit 0, nothing run. The same change under `src/` widens and runs everything |
...
`.jails` at build or test time.** *Today* above. *Change* S60.7's six
steps, plus: the ArchUnit freeze store moves to
`src/test/resources/archunit/frozen`, the architecture policy into
`jails.toml`, and every generated file carries the provenance header.
*Done when* `rm -rf .jails && mvn test` on a scaffolded project passes
with the same test count, `grep -rl '\.jails' src` is empty, and `ls` at
the root reads like any Maven project.

**I71.44 — `.jails/` is the input, and only the input.** *Today* it is the
input, half the output, a 21 MB lock and a scratch directory. *Change* it
holds `model.jdl`, the lock as a manifest of path, artifact id and digest,
and the merge base as a tree of files (I70.22); `run/` and `apply.lock`
are ignored. Deleting it removes the ability to regenerate and merge,
nothing else, the way deleting `schema.prisma` does, and the next `jails
g` says so: *no model at `.jails/model.jdl`; this project was generated by
jails and its model is gone; restore it from git*. *Done when* a project
with `.jails` deleted builds, and that refusal prints.
...
the build (the build-helper block and the Gradle source set already name
the roots, and `editor handshake` already prints the shape), consumed by
all five; after I71.43 the list is the standard one and the function
stays. *Done when* the table above has one row and `kafka describe` finds
the topic `g event` wrote.
...
**I71.25 — the warm refusal names the real reason.** *Today* *pass its
fully qualified test class*, which changes nothing. *Change* the refusal
says where the class lives and that the warm engine does not look there.
*Done when* it names `.jails/generated/test/java`, until I71.43 removes
the case.
...
*Change* `.gitattributes`: `.jails/compiler.lock.json -diff merge=binary`;
`.gitignore`: `.jails/run/` and `.jails/apply.lock`; README documents `rm
.jails/compiler.lock.json && jails sync` as the merge resolution with the
re-baseline caveat. *Done when* a fresh `new` carries both files and the
lock never appears in a `git diff`.
