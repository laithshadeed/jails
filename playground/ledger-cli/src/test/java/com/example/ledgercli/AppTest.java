...
class AppTest {
     ...
     */
    private SequencedMap<String, App.Command> commands() {
        var commands = new LinkedHashMap<String, App.Command>();
    ...
    private int run(String... args) {
        return App.run(commands(), new PrintStream(out), new PrintStream(err), args);
    }
    ...
    void namesTheUnknownCommandAndExitsTwo() {
        assertThat(run("nope")).isEqualTo(App.USAGE_ERROR);
