...
class LedgerCliTest {
     ...
     */
    private SequencedMap<String, LedgerCli.Command> commands() {
        var commands = new LinkedHashMap<String, LedgerCli.Command>();
    ...
    private int run(String... args) {
        return LedgerCli.run(commands(), new PrintStream(out), new PrintStream(err), args);
    }
    ...
    void namesTheUnknownCommandAndExitsTwo() {
        assertThat(run("nope")).isEqualTo(LedgerCli.USAGE_ERROR);
