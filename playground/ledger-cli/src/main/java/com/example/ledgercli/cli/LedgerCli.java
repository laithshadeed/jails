...
/**
 * Argv dispatch for the ledger command line: it owns argument routing, exit
 ...
 * int code = LedgerCli.run(LedgerCli.commands(), new PrintStream(out), System.err, "greet", "world");
 * }
 */
public final class LedgerCli {
    ...
    public static SequencedMap<String, Command> commands() {
        ...
        commands.put(ReconcileCommand.NAME, ReconcileCommand::run);
        return commands;
    }
    ...
    private static void usage(SequencedMap<String, Command> commands, PrintStream to) {
        to.println("usage: ledger <command> [args]");
        to.println();
        to.println("commands:");
        to.println("  help");
