...
/**
 * Publishes {@link PageDiscoveredEvent}.
 *
 ...
 */
@Component
public class PageDiscoveredPublisher {

    private final KafkaTemplate<String, PageDiscoveredEvent> kafka;
    private final String topic;

    public PageDiscoveredPublisher(
            KafkaTemplate<String, PageDiscoveredEvent> kafka,
            @Value("${topics.page-discovered:page-discovered}") String topic) {
        this.kafka = kafka;
        this.topic = topic;
    }

    /** The returned acknowledgement lets durable callers mark success only after Kafka accepts it. */
    public CompletableFuture<SendResult<String, PageDiscoveredEvent>> publish(PageDiscoveredEvent event) {
