...
/**
 * HTTP for {@link CrawlRun}.
 *
 * <p>Speaks in {@link CrawlRunRequest} and {@link CrawlRunResponse} rather
 ...
 */
@RestController
@RequestMapping(CrawlRunController.PATH)
public class CrawlRunController {

    /** The collection this controller serves. */
    public static final String PATH = "/crawl-runs";

    private final CrawlRunService service;

    public CrawlRunController(CrawlRunService service) {
        this.service = Objects.requireNonNull(service, "service is required");
    }

    @GetMapping
    public List<CrawlRunResponse> list() {
        return service.findAll().stream().map(CrawlRunResponse::from).toList();
    }

    /** 404 rather than an empty 200: "no such thing" and "here is nothing" differ. */
    @GetMapping("/{id}")
    public ResponseEntity<CrawlRunResponse> byId(@PathVariable String id) {
    ...
    @PostMapping
    public ResponseEntity<CrawlRunResponse> create(@Valid @RequestBody CrawlRunRequest request) {
        CrawlRun created = service.create(request.toDomain());
