...
/**
 * HTTP for {@link CrawledPage}.
 *
 * <p>Speaks in {@link CrawledPageRequest} and {@link CrawledPageResponse} rather
 ...
 */
@RestController
@RequestMapping(CrawledPageController.PATH)
public class CrawledPageController {

    /** The collection this controller serves. */
    public static final String PATH = "/crawled-pages";

    private final CrawledPageService service;

    public CrawledPageController(CrawledPageService service) {
        this.service = Objects.requireNonNull(service, "service is required");
    }

    @GetMapping
    public List<CrawledPageResponse> list() {
        return service.findAll().stream().map(CrawledPageResponse::from).toList();
    }

    /** 404 rather than an empty 200: "no such thing" and "here is nothing" differ. */
    @GetMapping("/{id}")
    public ResponseEntity<CrawledPageResponse> byId(@PathVariable String id) {
        return service.findById(id)
                .map(CrawledPageResponse::from)
                .map(ResponseEntity::ok)
                .orElseGet(() -> ResponseEntity.notFound().build());
    }

    @PostMapping
    public ResponseEntity<CrawledPageResponse> create(@Valid @RequestBody CrawledPageRequest request) {
        CrawledPage created = service.create(request.toDomain());
