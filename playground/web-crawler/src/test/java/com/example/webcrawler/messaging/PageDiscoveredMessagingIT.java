...
import com.example.webcrawler.KafkaTestcontainersConfig;
...
@Import({KafkaTestcontainersConfig.class, PageDiscoveredMessagingIT.ProbeConfiguration.class, TestcontainersConfig.class})
class PageDiscoveredMessagingIT {

    @Autowired
    private PageDiscoveredPublisher publisher;
    ...
    void aPublishedEventIsConsumed() throws InterruptedException {
        PageDiscoveredEvent event = new PageDiscoveredEvent(UUID.fromString("00000000-0000-0000-0000-000000000001"),
    ...
    static class Probe {
        ...
        private final AtomicReference<PageDiscoveredEvent> last = new AtomicReference<>();
         ...
         */
        @KafkaListener(topics = "${topics.page-discovered:page-discovered}", groupId = "page-discovered-it-probe")
        void on(PageDiscoveredEvent event) {
            last.set(event);
            received.countDown();
        }
    }
