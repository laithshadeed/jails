package com.example.paymentsgateway.clients;
 ...
 */
@Component
public final class SafeAcquirerFetcher implements AcquirerFetcher {
    ...
    @Autowired
    public SafeAcquirerFetcher(
            @Value("${jails.fetchers.acquirer.connect-timeout:2s}") Duration connectTimeout,
            @Value("${jails.fetchers.acquirer.response-timeout:10s}") Duration responseTimeout,
            @Value("${jails.fetchers.acquirer.max-response-size:2MB}") DataSize maxResponseSize,
            @Value("${jails.fetchers.acquirer.max-redirects:3}") int maxRedirects,
            @Value("${jails.fetchers.acquirer.user-agent:jails-acquirer-fetcher/1.0}") String userAgent,
            @Value("${jails.fetchers.acquirer.allowed-content-types:text/html,application/xhtml+xml,text/plain}")
