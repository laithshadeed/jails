...
@Component
public class JdbcAuthorisePaymentOutbox {

    private final JdbcClient db;
    private final int maxAttempts;
    private final int leaseSeconds;

    public JdbcAuthorisePaymentOutbox(
            JdbcClient db,
            @Value("${outbox.authorise-payment.max-attempts:10}") int maxAttempts,
            @Value("${outbox.authorise-payment.lease-seconds:30}") int leaseSeconds) {
    ...
    @Transactional
    public void stage(PaymentAuthorisedEvent event) {
        Objects.requireNonNull(event, "event is required");
        String payload = Json.toJson(event);
        int inserted = db.sql("""
                        insert into authorise_payment_outbox (id, payload, state, attempts, max_attempts,
                ...
                .update();
        if (inserted == 0) {
            var existing = db.sql("select payload::text from authorise_payment_outbox where id = :id")
                    .param("id", event.id()).query(String.class).single();
            if (!Json.parse(existing, PaymentAuthorisedEvent.class).equals(event)) {
    ...
    public Optional<Status> status(UUID id) {
        return db.sql("""
                        ...
                        from authorise_payment_outbox where id = :id
                        """)
                .param("id", id)
    ...
    public Optional<Claimed> claim() {
        return db.sql("""
                        with candidate as (
                            select id from authorise_payment_outbox
                            where (state = 'PENDING' and next_attempt_at <= now())
                               or (state = 'RUNNING' and lease_until <= now())
                            order by next_attempt_at, created_at
                            for update skip locked limit 1
                        )
                        update authorise_payment_outbox events
                ...
                .query((rows, rowNumber) -> new Claimed(
                        rows.getObject("id", UUID.class),
                        Json.parse(rows.getString("payload"), PaymentAuthorisedEvent.class),
    ...
    public void succeed(UUID id) {
        db.sql("""
                        update authorise_payment_outbox set state = 'SUCCEEDED', lease_until = null,
    ...
    public void fail(UUID id, RuntimeException failure) {
        ...
        db.sql("""
                        update authorise_payment_outbox
    ...
    public record Claimed(UUID id, PaymentAuthorisedEvent event, int attempt) {}
}
