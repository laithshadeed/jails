...
@Import({KafkaTestcontainersConfig.class, PaymentAuthorisedMessagingIT.ProbeConfiguration.class, TestcontainersConfig.class})
class PaymentAuthorisedMessagingIT {

    @Autowired
    private PaymentAuthorisedPublisher publisher;
    ...
    void aPublishedEventIsConsumed() throws InterruptedException {
        PaymentAuthorisedEvent event = new PaymentAuthorisedEvent(UUID.fromString("00000000-0000-0000-0000-000000000001"),
    ...
    static class Probe {
        ...
        private final AtomicReference<PaymentAuthorisedEvent> last = new AtomicReference<>();
         ...
         */
        @KafkaListener(topics = "${topics.payment-authorised:payment-authorised}", groupId = "payment-authorised-it-probe")
        void on(PaymentAuthorisedEvent event) {
            last.set(event);
            received.countDown();
        }
    }
