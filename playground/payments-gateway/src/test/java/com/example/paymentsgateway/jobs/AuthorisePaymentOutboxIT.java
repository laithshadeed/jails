package com.example.paymentsgateway.jobs;

import static org.assertj.core.api.Assertions.assertThat;

import com.example.paymentsgateway.KafkaTestcontainersConfig;
import com.example.paymentsgateway.TestcontainersConfig;
import com.example.paymentsgateway.app.PaymentRepository;
import com.example.paymentsgateway.domain.Payment;
import com.example.paymentsgateway.domain.PaymentMethod;
import com.example.paymentsgateway.service.AuthorisePaymentCommand;
import com.example.paymentsgateway.service.AuthorisePaymentUseCase;
import java.util.UUID;
import org.junit.jupiter.api.Test;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.test.context.SpringBootTest;
import org.springframework.context.annotation.Import;

@Import({KafkaTestcontainersConfig.class, TestcontainersConfig.class})
@SpringBootTest(properties = {
        "outbox.authorise-payment.initial-delay=PT1H",
        "outbox.authorise-payment.max-attempts=2"
})
@org.springframework.transaction.annotation.Transactional
class AuthorisePaymentOutboxIT {

    @Autowired private AuthorisePaymentUseCase useCase;
    @Autowired private PaymentRepository results;
    @Autowired private JdbcAuthorisePaymentOutbox outbox;
    @Autowired private AuthorisePaymentOutboxWorker worker;
    @Autowired private org.springframework.jdbc.core.simple.JdbcClient db;

    @Test
    void businessEffectAndEventAreStagedTogetherThenAllConfiguredSinksCompleteDelivery() {
        var command = new AuthorisePaymentCommand(
                UUID.fromString("00000000-0000-0000-0000-000000000001"),
                UUID.fromString("00000000-0000-0000-0000-000000000001"),
                "sample",
                1L,
                "sample",
                PaymentMethod.values()[0]);

        var result = useCase.execute(command);

        assertThat(results.findById(String.valueOf(result.id())))
                .get().extracting(Payment::id).isEqualTo(result.id());
        assertThat(outbox.status(result.id())).get()
                .extracting(JdbcAuthorisePaymentOutbox.Status::state)
                .isEqualTo(JdbcAuthorisePaymentOutbox.State.PENDING);

        worker.runOnce();

        assertThat(outbox.status(result.id())).get()
                .extracting(JdbcAuthorisePaymentOutbox.Status::state)
                .isEqualTo(JdbcAuthorisePaymentOutbox.State.SUCCEEDED);
    }

    @Test
    void retriesKeepTheStableEventIdAndTerminalFailureIsInspectable() {
        var command = new AuthorisePaymentCommand(
                UUID.fromString("00000000-0000-0000-0000-000000000001"),
                UUID.fromString("00000000-0000-0000-0000-000000000001"),
                "sample",
                1L,
                "sample",
                PaymentMethod.values()[0]);
        var result = useCase.execute(command);

        var first = outbox.claim().orElseThrow();
        outbox.fail(first.id(), new IllegalStateException("provider unavailable"));
        db.sql("update authorise_payment_outbox set next_attempt_at = now() where id = :id")
                .param("id", first.id()).update();
        var second = outbox.claim().orElseThrow();
        outbox.fail(second.id(), new IllegalStateException("provider unavailable"));

        assertThat(second.id()).isEqualTo(result.id()).isEqualTo(first.id());
        assertThat(outbox.status(result.id())).get().satisfies(status -> {
            assertThat(status.state()).isEqualTo(JdbcAuthorisePaymentOutbox.State.FAILED);
            assertThat(status.lastError()).contains("provider unavailable");
        });
    }
}
