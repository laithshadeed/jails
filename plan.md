...
| 7. Read-only `StateCompatibility` facade | partial | `jails_project::compat::read` classifies absent/current/legacy/unreadable without mutating, and `ledger::parse` refuses a newer schema with a no-downgrade message. But it is **entirely V1-shaped**: its `Current` is a V1 `Ledger` parsed by `ledger::load`, its `legacy_sources` lists only `app-state-v1`/`files`/`version`, and `.jails/ledger.toml` itself is treated as current rather than as something to translate. Nothing produces a `LegacyMigrationIdentity` — `migration` is `None` at every construction site — so the first-schema-2 commit `prepare` already validates has no producer. That is what blocks step 9. |
