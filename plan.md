...
| 4. `app init/plan/apply/reconcile` as one aggregate | partial | `route::app_apply` declares the whole manifest under `ReconcileScope::AppManifest` and commits once, so a row the manifest stops naming is relinquished rather than merely unmentioned. Each step plans against `Project::projected` -- a projection of everything before it -- which is what replaces the per-step reload: `g search` sees `add db`'s POM and `g scaffold`'s record without either having been written. `init`, `plan` and `reconcile` are not routed. |
