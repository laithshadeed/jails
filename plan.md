...
- **One more of that shape is still open**: `generate command` registers itself in the project's dispatcher, and `generate cli` retargets the POM's `<mainClass>`. Both are `std::fs` writes after the plan. `ResourceKey::CommandRegistration` and `SemanticEdit::CommandRegistration` already exist and the projection ignores the edit, so the fix is the same as the marked block's — state it, key it, and let the projection splice — plus moving `splice_registration`/`unsplice_registration` down beside `jails_java::annotate`, which is where the `@Import` splice already went for the same reason.
