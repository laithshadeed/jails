...
| 5. Maintenance mutations | done | Four of five routed as `PlannedSubject` maintenance subjects with empty `LedgerIntent`s. `rename` is the one with a real defect behind it: V1 rewrites contents then moves files and its own comment admits the half-applied state, while here a move is `Create`+`Delete` in one operation list and every source, destination and directory is a declared read. It also carries the **identity transition** §R6.4's row asks for -- an entity is named by its `IntentId`, so a renamed type's rows arrive at the new paths under the renamed id and the old id is removed in the same intent, which is what lets `destroy record Bonus` find what `rename Reward Bonus` moved. `adopt_layout` emits one `SemanticEdit::HumanConfigLayout` per adopted layer and lands one operation instead of one per layer -- keyed rather than a whole-file body because `jails.toml` has a second contributor (`[project] capabilities`, which `add` owns), so the two compose instead of one deciding every byte. `format` runs Spotless in a scratch tree synthesised from the projection and commits only what it changed inside its declared `mutable_scopes` -- so a formatter reaching outside `src/` is a refusal rather than a fait accompli, `target/` is skipped as derived output, and a run that changes nothing has no operations. `test --fast` is `ToolFeature::FastTest` under `DirectCli`, with `remove fast-test` as the same scope declaring nothing. `route::adopt_legacy` closes the last row. §R2.5 makes it the only path from an unknowable legacy manifest origin to a named owner, and the safety rule is the feature: jails re-renders the intent and requires what is on disk to be **byte-identical to the projection** — not to the recipe's artifacts and not to the contribution, since import order is normalised and blank lines tidied on the way to disk, so comparing anything earlier refuses every row jails itself wrote. Only then does the row become an owned entity with a truthful renderer stamp, which matters because adoption records those bytes as the row's *base* and every later merge measures from it. A key that names no row refuses rather than adopting something similar; the right key under the wrong intent refuses too. `doctor` lists every unowned row with its key and a copyable claim command -- derived in `compat::adoptable`, beside the route that accepts them, so a printed command cannot drift out of working. `--replace --force` closes the drifted case: the refusal now hands over the exact command, and that command installs the freshly rendered candidate with the current bytes as **guarded preimages**, so what was discarded is recorded rather than merely gone. The mechanism is deliberately not a force switch. §R5.3's unowned rule is lifted for a *set of paths* -- exactly the files the row's re-render produces -- because a flag on the transition would also lift it for every unrelated path the same commit happens to touch, and adoption is a statement about the files being taken over. |
...
| 7. Read-only `StateCompatibility` facade | done | `compat::read` classifies absent/current/legacy/unreadable without mutating -- and it now asks the schema-2 parser *first*, which it did not: the schema-1 parser refuses anything carrying a `schema` key, so the facade answered "use a newer jails" about a store this very binary had just written. `Store::observe` goes through that one classifier rather than repeating it, so there is a single opinion about whether a project needs migrating; what the commit path adds is the two things a reader does not need, the exact file image it guards under the lock and the legacy sources it retires. `ledger::parse` refuses a newer schema with a no-downgrade message, and `compat::translate` turns a schema-1 ledger into a schema-2 draft in memory — so `Store::observe` reads a project V1 built instead of refusing it. §R2.5's conservatism is the design: every schema-1 row becomes a `LegacyEntry` and **none** becomes an `AppliedEntity`, because the old format did not record who asked for a row and one whose fields match today's manifest is still of unknown origin. The draft is generation 0 with empty applied/one-shot/resource/output tables, so the first V2 mutation writes generation 1 with the schema-1 bytes as its guarded before-image. `jails.toml` is not translated: its capabilities become `DirectConfig` claims during ordinary resolution and migration never touches the file. The explicit deletion of the *other* legacy sources through a `LegacyMigrationIdentity` now lands with the migration. `Store::observe` records the complete pre-schema-2 source set in the same breath as the ledger -- two reads could describe two different machine states, and the commit would then guard preimages that were never all true at once -- preparation emits one guarded delete per source it found, and the executor performs them **after** the ledger rename. That order is the half that matters: before it, a crash would leave a project whose old registry is half deleted and whose new one was never written, which is losing rows; after it, a crash leaves files nothing reads any more. `translated_before` hashes the rows the translation produced rather than the draft, because a draft is generation 0 by construction and the envelope refuses to encode a generation nothing committed. A legacy target is also rechecked under the lock now, which was the one class of operation that reached the commit point unguarded. |
...
| 9. Flip the single dispatch point | ready, with one bill to settle first | The translation that blocked it exists (step 7). What settled *how* it must happen still holds: **all at once**, because one V2 command in a V1 project shares a ledger path with thirteen V1 ones and each writes a schema the other cannot read. Formerly: **blocked on the V1→V2 ledger translation, which did not exist.** The bill §R2.5's conservatism runs up is now visible and priced: after migrating, a schema-1 row has files and no owner, so `destroy` refuses it. That refusal names the row, says the old format never recorded who asked for it, and hands over the exact paths it will not delete; the route out is `route::adopt_legacy`, in both its plain and `--replace --force` forms, and step 5 is now closed. **The remaining bill is the CLI**: the closed option group §R6.4 specifies (`--legacy-key`/`--manifest`/`--intent`, with `--replace` requiring `--force`) is not wired into `main.rs`, deliberately -- wiring it before the flip would put one V2 command in a V1 project, which is the collision this row is about. It goes in with the flip, in the same commit, or `jails destroy` stops working on every project that predates schema 2. `.jails/ledger.toml` is one path with two incompatible schemas: `ledger::load` parses V1 and `LedgerV2::parse_file` parses V2, and neither reads the other. So a V2 route refuses any project that has ever run a V1 command — which is every real project — with `ledger has N line(s); schema 2 is exactly five`. This was found by flipping `adopt` and `rename` and running them against a project built by V1; both were reverted. **Measured again, after the translation landed**: flipping `generate` alone and running `tests/cli.rs` fails 25 of 165 tests, and the failures are behavioural rather than cosmetic. Three classes, each a real item, and **two of the three are now closed**. A `g cases`/`g migration` invocation reached `route::generate`, which has no arm for a one-shot: `route::recipe` is the one entry point from an intent to the route that owns its kind, closed on `ArtifactKind` so a kind added without deciding its policy is a compile error, and `Intent` is the shape both a manifest row and a command line arrive in — §R6.2 already says a direct call has the same direct-owner semantics as an equivalent manifest row, so they are one type. A project with no `pom.xml` was refused where V1 reported a degraded shape and continued: a dependency claim now splices into nothing rather than refusing, and the claim survives in the store, which is strictly better than V1's printed note because `doctor` can then say the dependency is absent; a *capability* still refuses first, through `require_maven`, because installing the code and skipping the dependency hands the reader a compile error for a line they did not write. What is left is `destroy` after a flipped `generate` failing outright, which is the schema collision above — inherent, and closed only by the atomic flip.

**A second probe flipped every mutating command** — `generate`, `add`, `remove`, `sync`, `rename`, `destroy`, `adopt`, `fmt` and the whole `app` aggregate — with §R3.4's rendering wired at the dispatch point, and was reverted after measuring. It compiled: `dead_code` is denied workspace-wide, so it also *forced* step 10's deletion, and `src/adopt.rs`, `src/app/reconcile.rs`, `src/app/shadow.rs` and V1's app-state reader all went with it (about 700 lines). Then 48 of 165 CLI tests, 2 of 4 agreement tests and the golden suite failed, and the failures name the rest of the work:

- The golden trees snapshot `.jails/`, and V2 puts an object store, a receipt directory and a lock there. Those are the executor's bookkeeping, named after content and transaction hashes; snapshotting them makes every golden churn on any change to any input. The collector has to exclude them, and rung 8's flat-`.jails` gate — written when the point was *one registry, not four* — has to be restated against what remains.
- The property block loses its `# jails:<capability>` markers, because V2 owns each property by `ResourceKey` and retires it by key. That is strictly better than deleting a marked block wholesale and diffing it for lines jails did not write, but it is a user-visible change to a file people edit and needs saying in `README.md` and `CLAUDE.md` before it ships.
- **Three real defects, all now fixed and none found by reading**: the V2 route wrote a `compose.yaml` whose service body was un-nested — valid-looking and not YAML — because the canonical mapping is stored dedented and the splice expects a body that carries its own nesting; a Spring project with compose services lost `spring-boot-docker-compose`, because that dependency was added by `add_in` rather than by the plan; and `g durable-job` stopped writing `src/test/resources/config/application.properties` entirely, because the block went in through a `std::fs` call *after* the plan and so was invisible to anything reasoning about a `Change`. All three are the same shape and it is the shape to look for in what remains: **a write V1 performs as a side effect of its own write path is a write the routes do not know about.** A `Change` states its marked blocks now, keyed by path and marker so removal takes exactly one out and the last one out takes the file with it.

- **The fourth is closed too.** `generate command` registered itself in the dispatcher with a `std::fs` write after the plan, so the routes wrote the command class and left it unreachable — a command nothing dispatches is a file, not a command. It is a keyed claim now: `ResourceKey::CommandRegistration` was already in the protocol and the projection ignored the edit, so the change was to make the projection splice it, move `splice_registration`/`unsplice_registration` down into `jails_java::dispatch` beside where the `@Import` splice went for the same reason, and give the recipe a way to *find* the dispatcher through the projection — in an aggregate apply, the `g cli` row that creates it and the `g command` row that registers into it are one transition, so disk does not have it yet. The dispatcher is not owned by the command: claiming it whole would make `destroy command` delete the CLI.

  What is left of this shape is `generate cli`'s retargeting of the POM's `<mainClass>`, which is the same kind of write and has no protocol key yet.

So the flip is: the dispatch, the golden collector, the two documentation changes, and roughly forty CLI assertions that pin V1's exact wording. It is one commit by construction — one V2 command in a V1 project is the collision this row is about — and it is the last thing before §R6.8. It also settles *how* the flip must happen: **all at once**, since one V2 command in a V1 project is the same collision. Everything else is ready — every existing V1 mutation path is routed and `--pretend` is a `Run` mode honoured in one place. `--name`/`--package` for capability instances is **done**: §R1.1's three classes are enforced at `CapabilityId::resolve`, every route that forms a capability identity now goes through one `Declaration` rather than hardcoding `Singleton`, and `jails.toml` grew the `[[capability]]` table those parameters need. That table was not optional decoration — without it a named capability could be installed and the next `sync` would retire it, because `DirectConfig` speaks for the *whole* list and a label rebuilt as a singleton is a different entity from the row `add` recorded. Reporting is closed on the side that exists: `--pretend` now returns §R3.4's `Report` and `CommandEnvelope::preview` derives the status from it, replacing a hand-rolled list that had already drifted three ways -- it called a replace an `update`, sorted by path where the executor keeps its own order, and dropped directory creation entirely. The committed side of the envelope is deliberately not projected yet: it needs the recovery vector and the replan-once loop this section specifies, and half an envelope would mean a `status` that is right for a preview and invented for everything else. `no_start` now reaches the canonical request: `Run::without_start()` carries it and `add`/`remove`/`sync`/`app apply` record it, so `add db` and `add db --no-start` are two invocations rather than one -- they were the same fingerprint before, and a resume comparing fingerprints would have accepted either as a continuation of the other. `ComposeReconcile` is built and run. Preparation constructs the descriptor by §R3.3's existence rule, the commit records it in the receipt as `Deferred` under §R4.2's `EffectId`, and `jails_commit::runtime` attempts it after the project lock is released -- §R6.6's ordering, and not an optimisation: an image pull would otherwise hold the mutation lock for the whole tree. `--debug` reaches it, that being the one subprocess a mutation route runs. Two readings this row previously called ambiguous are settled from §R3.3's own text rather than guessed. "Marker-free" means free of **conflict** markers, not of jails' `# jails:` comments: the same paragraph calls `after_document` the *exact* committed postimage, which markers-stripped bytes would not be, and a conflicted apply is separately specified to emit no executable effect at all. And the attempt sits after `execute::commit` returns, because `CommittedResult.effect` is the outcome of an attempt -- a commit naming one before making it would be claiming an outcome nothing produced. The committed envelope is closed too: `Outcome::envelope()` projects a preview from the prepared report and a commit from the published receipt, deriving the status from those rather than asserting it beside them, and `commit` runs §R3.4's replan-once loop -- a `RecoveredPriorTransaction` is a stale plan rather than a failure, so the store is reread and the request measured against it again, and a *second* such answer refuses rather than looping. The recovery report types moved down to `jails-prepare` beside the other report shapes, because the envelope carries them and the envelope is below the commit layer. What remains at the boundary is `--output human|json` on the CLI, which lands with the flip because a V1 command has no envelope to render. The one thing the suite deliberately does not exercise is the attempt itself: running it starts a real container, and a suite that leaves a PostgreSQL behind when it passes is worse than one that says which half it covers. So the argument vector, the identity and the settled-state rules are unit tests, and the route that reaches them is an integration test over a capability with no container. |
