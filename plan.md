...
| 9. Flip the single dispatch point | ready | Nothing blocks it. Every existing V1 mutation path is routed, and `--pretend` — the last thing the routes could not do that `main.rs` needs — is now a `Run` mode honoured in one place rather than fourteen. `adopt --legacy-key` is *not* a blocker: it has no V1 to flip, being new surface on the schema-1 migration story. What remains is the mechanical work at the boundary: `--name`/`--package` for capability instances, `no_start`/`debug` post-commit behaviour, and reporting what was written. |
