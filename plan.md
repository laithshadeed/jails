...
- **Two real defects, both now fixed and neither found by reading**: the V2 route wrote a `compose.yaml` whose service body was un-nested — valid-looking and not YAML — because the canonical mapping is stored dedented and the splice expects a body that carries its own nesting; and a Spring project with compose services lost `spring-boot-docker-compose`, because that dependency was added by `add_in` rather than by the plan.
