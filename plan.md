...
about the current tree in §3 was re-checked with a grep or a read *today* —
...
generator surface (§4) ahead of the test daemon (§5), and puts eight live
defects (§3) ahead of both, because a tool that silently writes a broken
...
gets a command, branch, enum or template in core. §8 respects that; three of
the six documents do not.
...
| Change shape | Today | After §4 | Frequency |
|---|---|---|---|
...
## 2. Corrections ledger — do not build these
...
| **`-XX:+AllowEnhancedClassRedefinition` covers ~90% of real edits** (`ideas-opus.md` A1) | The flag does not exist in OpenJDK (`grep -rl` over `deps/jdk` returns nothing). It is JetBrains Runtime / DCEVM, and **JBR tops out at JDK 25** while `pom::TARGET_RELEASE` is 27 (class-file major 71 → `UnsupportedClassVersionError`). Stock JVMTI is method bodies only (`jvmti.xml:8136-8140`). Since jails' domain layer is records and sealed types, **every domain edit is a restart** — say so, or `jails dev` looks broken. |
...
| **`jails run --watch` already pipes through `why`** (`ideas-opus.md`) | It does not, and still does not — see §3.4. |
...
is `@since 6.2`, i.e. Boot ≥ 3.4 — which is why §3.3 exists.

---

## 3. Tier 0 — the live defects, verified today
...
## 4. Tier 1 — the authorship engine
...
### 4.1 `g field` — the single highest-value generator jails does not have
...
### 4.2 Relations — the only defect here whose symptom is lost data
...
### 4.3 The pluraliser is `+ "s"`, and it is visible in the URL
...
by hand is the same six-file spread as §4.1 for a typo you did not make.
...
### 4.4 One rule for where fields come from
...
### 4.6 `g factory` — the missing half of test speed
...
### 4.9 The manifest is the new ergonomic unit, and it cannot be edited
...
That is §4.1 one level up. **`g field` is the primitive the manifest needs to
...
### 4.10 What not to build here

- **No ORM, no relation traversal, no lazy loading.** §4.2 is a column and a
...
- **No provenance ledger as a prerequisite** — see §6.
...
## 5. Tier 2 — the latency engine

Ordered by measured value, with §2's corrections already applied.

### 5.1 Free wins, hours not days
...
(mandatory alongside §3.4), `Class#method`, `--fail-fast`
...
### 5.2 `jails test --fast` — get Maven out of the loop
...
1. **Watcher**: §3.6's replacement, 150–250 ms poll, 400 ms quiet period, plus
...
### 5.4 `jails run --tc` — the honest "console with beans"
...
### 5.5 Where AOT actually belongs
...
### 5.6 Explicitly out of the dev loop
...
the JVM saw**). JBR/DCEVM/HotswapAgent/JRebel (§2). The AOT cache (§2). Maven 4
...
## 6. Tier 3 — safe mutation, and exactly how much of it you need
...
**Resolution: ship §4 on `edited_files`; build provenance when `app apply`
...
- Provenance becomes load-bearing the moment `app apply` must reconcile a
  changed manifest line against a file on disk (§4.9), which is the natural
  trigger to build it.
...
## 7. Tier 4 — reach: the codebase you did not create
...
## 8. Tier 5 — the capability set, inside the domain-blind line
...
- **Traversal, assignment and extraction arrive as generic intents** —
  `usecase`, `query`, domain `event`, durable `job` — which is what
  `examples/DOGFOOD.md`'s friction ledger already commits to.
- **The bug list is the acceptance criteria** regardless of which command
  emits the code.

Ordered by what is blocking today, not by interest:

### 8.1 `add cors` — the actual blocker, and nobody mentioned it
...
### 8.2 `add sse` — with the four details both SSE designs get wrong

`SseEmitter` is alive and undeprecated in Framework 7; both documents pick it
and both are right about that.

- **The never-time-out value is `-1L`** (or `0L`), not `Long.MAX_VALUE` — §2.
...
### 8.3 `add queue` — the backbone both verticals silently assume
...
### 8.4 The rest, with the trap each one exists to remove
...
leaves red squiggles until prompted**), `autobuild.enabled = true` (§5.3
...
## 10. Tier 7 — the agent as second user
...
5. **Promote `g cases`.** It turns a markdown brief's acceptance bullets into
   ...
   is implemented, and §3.9 says nobody can find it.
...
## 11. Anti-goals — the union, with reasons
...
| Gradle *support* | Distinct from Gradle-directory *tolerance*, which is §7 and is worth having |
| ORM, JPA, `JpaRepository`, an Active Record clone, lazy loading | SQL stays derived and visible; §4.2 is a column and a foreign key |
...
| The AOT cache in `dev`/`test`; CRaC; JBR/DCEVM/HotswapAgent/JRebel | §2 |
...
| Redis required for the queue; Elasticsearch for v1 search | §8.3, §8.4 |
...
|---|---|---|---|---|
| 1 | Tier 0: all eight defects + the `mvn validate` matrix + the Lua-pinning test | §3 | 1 day | Features land on top of it. Two of them make jails write a broken project |
| 2 | Editor config: jdt.ls settings + bundles + HCR, `'path'`/`src.zip`, `:compiler jails`, keymap split, `javac_lint --release`/opt-in | §9 | S, no Rust | Test-at-cursor, debugging, `gf` and quickfix — today, with no Rust written |
| 3 | Testcontainers reuse + doctor + `jails setup`; `spring-devtools.properties`; `mise.toml` from `new` | §5.1 | S | −8 s per container test, −1.2 s per restart |
| 4 | `jails test` flags: `Class#method`, `path:line`, `--failed`, `--fail-fast`, `--slowest`, rerun snippet, `failIfNoSpecifiedTests=false` | §5.1 | S–M | Every test run |
| 5 | `why` on every Maven failure; `why --json`; doctor/why additions | §5.1, §10 | S | Multiplies every rule already in the table |
| 6 | **The inflector**, **scaffold reads the record off disk**, **refusal messages with `fix:`** | §4.3–4.4, §4.8 | S each | Half-day changes, all three visible in the first minute of a demo |
| 7 | **`g field`**, then **relations** | §4.1–4.2 | M each | The two that move jails from "makes a project" to "grows a project". `g field` first — §4.9 means `app apply` needs it before the manifest can become editable; relations second — the only defect whose symptom is lost data |
| 8 | `--timestamps`, `g factory`, `requests/*.http` | §4.5–4.7 | M total | Ride the same generator surface; do them together |
| 9 | `about --json` v2 + line numbers; projectionist; fzf-lua pickers; `jails src` | §9 | M | `:A`/`:E*` and routes/beans jump — 50–200 keystroke-saves a day |
| 10 | `jails test --fast` (console launcher + classpath cache) + `jails bench` | §5.2 | M | 2.5 s → ~0.5 s, and the first thing that proves its own number |
| 11 | §7 marker widening + `jails adopt` | §7 | M | Changes what the tool *is*: from "makes new projects" to "you can bring it to a codebase" |
| 12 | `jails testd` + `--affected` | §5.2 | L | The biggest single number (35×/345×) and deliberately not first — it has the one unmeasured piece, and an 11 ms test loop is worth little on a model you cannot change |
| 13 | `jails dev` v1: watcher, `javac`+CDS, trigger file, `why` piping, migrations, keys, timings | §5.3 | L | After #10/#12 prove the primitives |
| 14 | `add cors`, then `add sse` | §8.1–8.2 | S, M | `cors` is small and is what currently makes a jails project unusable from a browser |
| 15 | `add queue` | §8.3 | L | The shared dependency of mailer, webhook retry and any durable frontier. Before all three |
| 16 | `g auth`, `g webhook`, `add mail`, `g search`, `g timeline` | §8.4 | M each | In that order; each waits for a real need |
| 17 | `add crawl` (+ `deps.tsv` rows, cloned first) and the crawl `usecase` intent | §8.4 | L | After #4 and #10. A spider you test with `mvn clean verify` is the old experience with more files |
| 18 | Provenance / `ChangeSet`, `jails status`, `codemod.rs` | §6 | L | When `app apply` needs drift reconciliation, not before |
| 19 | `AGENTS.md` + `jails lint` + `--json` everywhere + `g cases` in README | §10 | M | Every agent session |
| 20 | `add ci`, `add docker`, `add aot`, `jails ship`, `jails upgrade`, `add arch`, `add nullcheck`, `new --offline`, `--module`, `g load`, `jails recap` | §5.5, §8.4 | S–M each | As they come up |

Items 1–6 are roughly a week of mostly small changes and they change the feel
of every subsequent hour. Items 7–8 are the ones you pay for on **every single
model change**. Item 12 is the biggest number and is correctly third.
...
6. Where jdt.ls writes `.class` files in this setup — `target/classes` via m2e
   ...
   whole §5.3 "the loop already exists" finding pivots on it.**
...
### One decision to record rather than leave implied

**`pom::TARGET_RELEASE = 27`.** JDK 27 is non-LTS (six months of updates) and
not GA until 2026-09-15. The costs are concrete and already being paid: **11
of 104 integration tests skip and the suite still says green**; mise's java
registry carries no JDK 27 build so an EA build is symlinked in; JBR tops out
at 25 so enhanced redefinition is blocked (§2); there is no GA
`eclipse-temurin:27-jre` image, so `add docker` needs a `jlink` stage; and
`doctor` FAILs by default on any shell without the mise hook. Java 25 is LTS,
is JBR's ceiling, is Boot's AOT-cache floor, and is where compact source
files, module imports, flexible constructors and `ScopedValue` all went final
— **everything this plan uses is available at 25.**

Two defensible answers: default to 25 with `--java 27` opt-in, or keep 27 and
write the reasons into CLAUDE.md next to the pin. What is not defensible is
leaving it implied, because three of the six source documents independently
flagged it and each spent effort rediscovering the same consequences.
...
- **`ideas-opus.md`** — the loop-latency framing, `jails dev`, the sub-second
  ...
  the dev loop, enhanced class redefinition) are dead; see §2. Everything else
  survives.
...
- **`ideas-sol.md`** — the `ChangeSet`/provenance/journal design, the CLI
  ...
  this plan overrules it, in §6.
- **`ideas-fable.md`** — twelve research passes with `file:line` citations. The
  correction table in §2 is largely its work, as is the jdt.ls/HCR path, the
...
- **`ideas-opus2.md`** — the only document that ran the tool. Every number in
  §1 and every defect in §3 traces to it. It is also where the authorship
  budget (§1.2–1.3) comes from, which is the axis this plan is organised
  around.

**Re-verified against `94107ef` while writing this file**: §3.1 (`generate.rs:1029,1059`,
`spring.rs:29`), §3.2 (`tests/golden/scaffold-plain/pom.xml:14-17`), §3.4
(`run.rs:162-180`), §3.5 (`run.rs:83,264,372`), §3.6 (`run.rs:325`), §3.9
(`grep -c cases README.md` → 1; `toxiproxy`/`app` absent from
`jails.nvim/lua/jails/init.lua`), §4.3 (`sql.rs:288-295`), §4.4
(`generate.rs:710,789`), §7 (`generate.rs:86-96`), §8.1 (zero `cors` matches
in `src/`, `templates/`, `README.md`), §5.1 (zero `withReuse` matches), §4.5
(no `--timestamps` in the clap definition), and the enum inventories (22
`ArtifactKind` variants, 16 `Capability` variants). **One defect the source
documents report is already fixed**: `doctor` now resolves `JAVA_HOME` before
PATH (`doctor.rs:871-875`).
...
in those sessions, not this one); the upstream `deps/` line numbers, which
drift — prefer the stable anchors (function names, enum variants, the grep
itself); and everything in §13.
