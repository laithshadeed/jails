...
enum OneShotSpec {
    ...
    Cases { source: SourceInputId, source_sha256: ObjectId,
            output: ProjectPath },
    Manifest { path: ProjectPath, body: ObjectId },
}
...
| One-shot manifest seed | `app init` | `OneShotSpec::Manifest { path, body }`; one file with fixed bytes, refused rather than merged when the path is already there. It is a one-shot because jails does not own the result: the reader edits the manifest, `app apply` reads it, and nothing regenerates it -- so there is no desired state to reconcile and no destroy route, since everything the manifest was used to apply outlives the file it was declared in. The path is a **declared read**, so the refusal is rechecked under the lock rather than being a `Path::exists` with a write after it. |
...
| `OneShotId` / `OneShotSpec` | field `0`, migration `1`, cases `2`, manifest `3` |
| `OneShotState` | active `0`, retired-target-removed `1` |
| `OneShotLifecycle` | field `0`, migration `1`, cases `2`, manifest `3` |
...
| 4. `app init/plan/apply/reconcile` as one aggregate | done | All four routed. `route::app_apply` declares the whole manifest under `ReconcileScope::AppManifest` and commits once, so a row the manifest stops naming is relinquished rather than merely unmentioned. Each step plans against `Project::projected` -- a projection of everything before it -- which is what replaces the per-step reload: `g search` sees `add db`'s POM and `g scaffold`'s record without either having been written; the falsifier is `examples/web-crawler`'s eleven capabilities and eleven intents as one transition. `route::app_plan` is that same computation stopped one step before the lock, so `--pretend` names exactly the files the apply then writes -- there is no second walk to disagree with, which is what retires the shadow comparison. `route::app_init` is a `OneShotSpec::Manifest` receipt whose path is a declared read, so "already exists" is rechecked under the lock instead of being a `Path::exists` with a write after it. **`reconcile` needed no route**: §R5.3's decision table already three-way merges any rewrite with a recorded base, so an edited file and a changed manifest row merge for free, and a genuine overlap refuses without writing -- both pinned by tests. |
...
| 9. Flip the single dispatch point | not started | Blocked on 5. |
