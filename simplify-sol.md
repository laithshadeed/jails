...
| `jails new` (Spring) | **the gap that is left, and it is the headline command.** `new-cli` seeds `.jails/model.jdl`; `spring.rs` does not, so every Spring project a reader creates is legacy with a ledger. The seeding is one line. Attempting it found six things and closed four -- see below -- and the two that remain are not one-liners. **First**, `new` writes six default properties into `application.properties` as reader-owned text, and a capability that later declares the same key refuses: "reader-owned properties already declare `server.shutdown`". `default_properties` has documented the cure since it became one owner for two writers -- seed them as `prop` declarations so the compiler owns the key -- and only the writer was ever built. The cost is that the explanatory comment above each default has nowhere to live in the model, which is a real loss to weigh rather than a detail. **Second**, and the one that actually blocks it: the canonical compiler requires a *declared* default where the legacy generator inferred one. `usecase SendMessage --on Message` carries three of `Message`'s seven fields, and the compiler answers "cannot construct required field `is_read` -- carry it in the command or declare a typed field default". That is the better rule, so the fix is in the proof manifests rather than the compiler: `examples/minicom` and its siblings have to declare what `isRead`, `timeStamp` and `version` are when a command does not carry them. Every manifest in `examples/` needs that pass before the flip can be green. |
