...
| `jails new` (Spring) | **the gap that is left, and it is the headline command.** `new-cli` seeds `.jails/model.jdl` and is canonical; `spring.rs` does not call `seed_canonical_model` at all, so `jails new` still produces a legacy project with a ledger. Every Spring project a reader creates is therefore legacy, which is what keeps the engine load-bearing after `adopt` and `modernize`. Flipping it is the same shape as flipping the plain toolbox was -- a fixture that had never run the canonical path, cascading into every test built on it -- and that flip found three real defects, so this one should be expected to find its own rather than treated as a switch. |
