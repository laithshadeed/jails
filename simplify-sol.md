...
| `new --app` | **closed.** It seeds a model like every other `new-cli` project and replays the manifest into it, at the root of the tree being published. The root rides on `Invocation` rather than on nine new parameters: `jails new` stands in the parent of the project it is creating, `Invocation` is the resolved value this rung asks a fact to be read off, and it was already threaded everywhere. Two `_at` functions stop the walk. **What it was worth is what it exposed**: the shared plain toolbox is built this way and had therefore never run the canonical path, and flipping it found three defects nothing else could see -- `currency:Currency` resolving to `java.util.Currency` beside the project's own `enum Currency`, a generated CLI whose `commands()` registered nothing and answered `unknown command` at run time, and three companion tests shipping `@Disabled` over records that had just been told what they guarantee. |
| `jails new` (Spring) | **the gap that is left, and it is the headline command.** `new-cli` seeds `.jails/model.jdl`; `spring.rs` does not, so every Spring project a reader creates is legacy with a ledger. The seeding is one line and has now been attempted twice. It is close: the minicom manifest applies canonically and `mvn test-compile` passes. Ten defects were found and fixed on the way, none of which any existing test could see -- four operation flags the frontend refused to translate (`--via`, joined filters, `--order-by ... desc`, `?` optional filters, `--on-conflict`), a `final` `@Transactional` adapter Spring cannot proxy, a generated `@SpringBootTest` with no container that silently used a developer's own database, two idempotency holes a replay finds, a v1 index grammar, and an enum with no SQL type. **What remains is not a defect but a measurement**, and it has moved twice since it was written here. For the same manifest the canonical path produced 8 Surefire reports and 23 tests against the legacy engine's 21 and 57; the missing ones were the per-operation controller tests the comments in `examples.rs` enumerate by name, and the flip was blocked on whether the canonical `api` capability should emit them. **It does now** -- each controller carries a companion test issuing a real request -- and `302d0b6` measures 15 reports and 33 tests. The legacy side is pinned in `examples.rs`; the canonical side is not pinned anywhere, so re-measure it rather than quoting this line. Lowering the pin would silently accept losing the difference, which is what `skipped: 0` exists to prevent. Two smaller things go with it: `new` must seed its six default properties as `prop` declarations rather than writing them as reader-owned text (the collision is real and the cure is designed but unbuilt, and it costs the explanatory comment above each default), and the proof manifests must declare the defaults the compiler will not infer -- `usecase SendMessage` carries three of `Message`'s seven fields, and "cannot construct required field `is_read`" is the better rule. |
...
2. ~~Retire `.jails/app.toml`.~~ **Mostly done, and by making it an importer
   ...
   and `new --app` seeds a model and replays into it like any other project.

   What remains under this heading is not the manifest at all: **`jails new`
   still creates a legacy project.** `new-cli` seeds `.jails/model.jdl`;
   `spring.rs` never calls `seed_canonical_model`, so every Spring project a
   reader creates has a ledger and takes the legacy path. That is what keeps
   the engine load-bearing once `adopt` and `modernize` are dealt with.
