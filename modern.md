...
Three of the four are closed (plan.md P5.4): a `!` component carries
`check (length(btrim(x)) > 0)`, an `@unique` `email` gets a case-insensitive
unique index, and the foreign key's two unobvious choices are stated in the
migration itself. What survives is the first bullet, which is an input
problem rather than a jails one:
...
### 6.1 The service layer calls a JDBC class directly
...
- **`ApiExceptionHandler`** as a design — `ResponseEntityExceptionHandler`,
  ...
  name back to an unauthenticated client. It has something to throw into it
  now: a generated transition raises `ApiException.NotFound` /
  `ApiException.Conflict` when the project installed the model, and keeps its
  own `ResponseStatusException` mapping when it did not — where the class does
  not exist and nothing else would compile.
...
- **`timestamp` as a column name** (2025-11-21, 2026-01-09, 2026-02-05). Legal
  in Postgres, but it is a type name; `sentAt`/`createdAt` costs nothing. An
  input problem: jails renders the name it was given.

The other two are closed. `deferrable initially deferred` is still the
default and the migration now says what it buys and what it costs, which is
the first of the two branches this entry offered. `@unique` on an
