 ...
 */
@Component
public class Jdbc{{name}}Presence implements {{name}}Presence {
    ...
    public Jdbc{{name}}Presence(
            JdbcClient db,
            @Value("${presence.{{property}}.window:PT30S}") Duration window) {
    ...
    public void heartbeat(String scope, String member) {
        db.sql("""
                        insert into {{table}} (scope, member, node, seen_at)
    ...
    public void leave(String scope, String member) {
        db.sql("delete from {{table}} where scope = :scope and member = :member and node = :node")
                .param("scope", scope)
                .param("member", member)
                .param("node", node)
                .update();
    ...
    @Scheduled(fixedDelayString = "${presence.{{property}}.sweep:PT60S}")
    public void sweep() {
        db.sql("delete from {{table}} where seen_at < now() - make_interval(secs => :seconds)")
