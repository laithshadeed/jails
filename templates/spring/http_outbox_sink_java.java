...
@Component
@Order(100)
@ConditionalOnProperty(name = "{{property}}.url")
public final class {{name}}HttpOutboxSink implements {{usecase}}OutboxSink {
    ...
    public {{name}}HttpOutboxSink(
            @Value("{{url_value}}") String url,
            @Value("{{bearer_token_value}}") String bearerToken,
            @Value("{{connect_timeout_value}}") int connectTimeoutMillis,
            @Value("{{request_timeout_value}}") int requestTimeoutMillis,
