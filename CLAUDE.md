...
**The lock names the merge base; the base is a tree of files.**
`jails.compiler-lock.v5` carries a `base` manifest -- one row per managed
path with its kind, mode, provenance and digest -- and the accepted bytes
are files under `.jails/base/<path>`, published by an ordinary
`PublishMergedTree` through the one executor. Capture reads them back,
rebuilds the `RenderedTree` and recomputes `projection_digest`, so the rule
that has held since v2 is unchanged: **the digest is of the form `serde`
derives from the whole tree**, whatever shape the file kept it in. Every
earlier schema still decodes, which is what lets a project upgrade by being
used. The base is published when it differs *or is absent*, and absence is
read from the snapshot: a v4 lock decodes to exactly this projection, so
equality alone would migrate the schema and leave no base beside it.
`.jails/` is jails' own record, not anybody's source -- the external-type
index, `jails src` and two test walkers each skip it, because a stale copy
of a type answers "does this exist" with a file that is about to stop
existing.
...
migrate. Skipping it took a hundred-entity mutation's materialize phase
from 116 ms to 30 ms.
...
**`.jails/` is the input, and nothing generated reads it.** No generated
file names the state directory at build or test time: the ArchUnit freeze
store is `src/test/resources/archunit/frozen`, an ordinary checked-in test
resource, and the reviewed exceptions are `[[architecture.allow]]` tables in
...
is green with the *same test count*. **Deleting `.jails` is then a lost input,
not a lost application, and the next command says so**: `model_init::
refuse_if_modelled` refuses when a source under `src/` carries the
`// Generated by jails from art_...` header and no model stands beside it,
because seeding a fresh one there reads the whole generated tree as the
reader's and collides several commands later. That header is on every managed
file, which is why one is enough -- `JavaUnit::render` writes it and
`ArchitectureTest.java` goes through the same shell as everything else.
...
- **`crates/jails-support/src/apply/`** -- **the only module that writes.**
  ...
  `ensure_directory_outside_project`, `put_in_scratch`, `remove_derived`,
  `ensure_derived_directory` and `mark_state_scratch` are the named
  exemptions; the two `derived` ones refuse a
  path outside `target/` or `build/`, and the last writes exactly
  `.jails/.gitignore` -- `apply.lock` and `run/`, so the mutex and the
  daemon's scratch never reach a diff. The executor calls it *after* the
  transaction, so a refused plan still writes nothing. `apply::Tree` is exempt
  as a type: a function taking one cannot reach a published project.
