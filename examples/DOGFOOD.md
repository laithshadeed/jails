...
| generate | `g command Reconcile` left the command unregistered: `new-cli` writes `App.java` and `g cli Ledger` writes a second dispatcher, so the project had two and jails would not guess. The note did not even say which two | `--on <Dispatcher>` names it (`on` in a manifest), an unknown name is a refusal listing the real candidates, and the ambiguous note now names every dispatcher it found |
...
| Ledger CLI | manifest | a second dispatcher has to be named by hand (`on = "Ledger"`), because "the one you meant" is genuinely unknowable | fine as it stands -- but `g cli` could record itself as the project's default dispatcher, so only a project with three needs the flag |
