...
## App C -- the payments gateway, 2026-08-22

Money, idempotency, tenancy and durable settlement, with **no payment,
merchant or settlement noun anywhere in `src/`**: money is `amountMinor:long`
plus a currency string, idempotency is a `@unique` column, ownership is two
generic `association` intents, and settlement is an outbox plus a
`durable-job`.

```bash
jails new payments-gateway --deps web,validation
mkdir -p payments-gateway/.jails
cp examples/payments-gateway/.jails/app.toml payments-gateway/.jails/app.toml
cd payments-gateway
jails app plan            # writes nothing
jails app apply --no-start
jails routes && jails beans && jails stats
jails doctor              # 15 checks, all clear
jails migrate --check     # 7 migrations applied to a scratch database
jails check               # mvn clean verify
```

**Result: BUILD SUCCESS in 1 m 45 s** -- 66 unit tests and 17 integration
tests against real PostgreSQL and Kafka, 0 failures, 3 skipped. 80 manifest
lines produced **6,429 lines across 137 files** (80x), with **zero
hand-written Java or SQL**. `routes` reports ten generated endpoints; `beans`
resolves 44.

| Attempt | Observable failure | Generic fix made in Jails |
|---|---|---|
| generate | `usecase AuthorisePayment --yields PaymentAuthorised` refused: it could not derive the event's `paymentId`. The event cannot call it `id` -- that is the event's own identity, which the outbox requires to be a separate required UUID -- so the identity of the row the use case just created had no spelling at all | an event field named `<Target>Id` now resolves to `result.id()`, the same `<parent>Id` convention `association`, `durable-job` and every scaffold already use. The refusal, when it still fires, names that spelling as one of the three options |
| doctor | on a project with both `add db` and `g event`, the test-datasource check named the **wrong class** (`PaymentAuthorisedMessagingIT`) and then reported 13 correctly-wired tests as broken, including `TestcontainersConfig` failing to import itself | the check reads annotations instead of bytes: a `@TestConfiguration` nested inside a `@SpringBootTest` is that test's private wiring and not the project's config, and `@SpringBootTest` inside `{@code ...}` Javadoc is not a test |
| doctor | the same check missed `@Import({SomeIT.Containers.class, TestcontainersConfig.class})` -- the *merged* form jails' own splicer produces -- because it matched the literal string `@Import(TestcontainersConfig.class)` | membership is now tested against the annotation's argument list. A check that goes red on correctly wired code is worse than no check: the fix it names changes nothing |

The manifest in `plan.md` §4.3 was a specification, and running it corrected
it twice: the `event` intent has to be declared **before** the use case that
yields it, since intents run in manifest order, and a `durable-job`'s fields
are the command's fields in order rather than a summary of them.

### What App C still does not prove

Its §4.3 contract has clauses nothing generated here satisfies, and they are
recorded rather than quietly dropped:

- **a retained idempotency result.** `idempotencyKey:string!@unique` gives
  one-payment-per-key; it does not return the first result to a duplicate
  request or 409 a different payload under the same key. This is the first
  genuinely new generic intent App C demands -- `g idempotency` (§13.3).
- **SLO histogram buckets and a `ping`-only liveness probe** (§5.2), and the
  datasource pool/`pg_is_in_recovery` defaults (§5.3).
- **a k6 profile and a recorded p99** (§5.5).
- **`createdAt:instant` is hand-declared on all three scaffolds**, which is
  §9.5's `--timestamps`.

## App D -- the ledger CLI, 2026-08-22

The control application: plain Maven, no Spring, no web, no database. Its
whole purpose is that A, B and C are all Spring Boot, so any Spring-shaped
assumption in the generic machinery is invisible to them.

```bash
jails new-cli ledger-cli
mkdir -p ledger-cli/.jails
cp examples/ledger-cli/.jails/app.toml ledger-cli/.jails/app.toml
cd ledger-cli
jails app plan
jails app apply --no-start
jails doctor          # 9 checks, all clear
jails check           # mvn clean verify
```

**Result: 53 tests, 0 failures, 6 skipped, BUILD SUCCESS in 1.9 s.** 31
manifest lines produced 1,633 lines across 38 files -- 53x -- with **zero
hand-written Java or SQL**. `doctor` reported nine checks and no failures.

It also disproved one prediction: `plan.md` §4.4 expected D to walk straight
into defect 8.1 (a `pom.xml` Maven cannot parse, from the versionless
validation starter). It does not, because 8.1 is reached through `g scaffold`
and `g dto`, and a CLI manifest asks for neither. `mvn -o validate` passes.
8.1 is still real; D is simply not the thing that proves it.

| Attempt | Observable failure | Generic fix made in Jails |
|---|---|---|
| verify | `jails check` failed on a freshly generated project: `add format` installs Spotless and the four files jails wrote itself violated it -- a `package-info.java` ending on a blank line, and `class NoteTest {` followed by two | `write_new_file` now runs every generated `.java` through `tidy_blank_lines`, which collapses consecutive blank lines and ends the file with exactly one newline, **leaving text blocks alone** because a blank line inside one is data. Pinned as a golden property, and it fixed the same latent violation in 18 snapshots |
| verify | the formatter had run *before* every generated file existed. `app apply` installs capabilities first, then runs the generate intents, so on a manifest naming `format` nothing jails generates is ever formatted; the second reconcile pass printed "already set up -- nothing to do" and returned | the already-set-up path for `format` re-runs the formatter instead of returning early, so the reconcile pass finishes the capability's actual work rather than just its pom edit |
| generate | `g command Reconcile` left the command unregistered: `new-cli` writes `App.java` and `g cli Ledger` writes a second dispatcher, so the project had two and jails would not guess. The note did not even say which two | `--on <Dispatcher>` names it (`strategy_on` in a manifest), an unknown name is a refusal listing the real candidates, and the ambiguous note now names every dispatcher it found |
| test | `EntryTest` arrived `@Disabled` because jails "cannot know how to build" an `amount:Money` -- where `Money` is a value object **jails generated two intents earlier** | a component whose type is a record already on disk is now sampled by reading it, recursively and depth-bounded, with the nested components' imports carried along. Scoped to code generated into the record's own package, since the call is unqualified. The three remaining skips are the strategy TODO bodies, which are the reader's work |

## The harness ran out of disk, again

2026-08-22, mid-sweep: `/tmp` hit its quota and **every** shell command
started failing, not just Maven's. The cause is the one already in the
friction ledger — `tests/common/mod.rs::temp_dir` creates a whole Maven
project per test cell and nothing ever removes it, so six full sweeps plus
two proof applications is enough to fill the disk.

It is recorded here rather than only fixed because the shape matters: the
first symptom was two `app_manifests_*` tests failing, which reads exactly
like a code regression and is not one. **A green suite that turns red when
the disk fills is indistinguishable from one that turns red on a bug** until
you look at the error.

The fix keeps the property the accumulation was paying for: when a
real-toolchain test fails, the generated project *is* the evidence, and a
`Drop` guard would delete it before anyone could read it. So `temp_dir` now
sweeps once per process and removes only fixtures older than an hour — well
...
| Payments gateway | tests | the scaffold's `Jdbc<Name>RepositoryIT` is a stub, `@Disabled` with *"todo: configure the test database"* -- advice that is already false in any project that ran `add db`, and the only three skips in the gate. `g query` generates a real round-trip IT for the same table | generate the real repository round trip when a container config is on the test classpath, and reserve the disabled stub for projects without one |
| Payments gateway | manifest | a `durable-job`'s fields must repeat its command's, exactly and in order, because `@scope` markers do not survive into the generated command record and deriving them would silently drop tenant enforcement | make constraints readable back off the generated source (or recorded per §11.2), then derive the payload from `--on` |
| Ledger CLI | manifest | a second dispatcher has to be named by hand (`strategy_on = "Ledger"`), because "the one you meant" is genuinely unknowable | fine as it stands -- but `g cli` could record itself as the project's default dispatcher, so only a project with three needs the flag |
| Ledger CLI | strategy | three of the six skipped tests are `g strategy` implementations whose bodies are `TODO`, which is correct, but the *test* is disabled rather than the implementation | consider generating an executable pending test that fails loudly, the way `g cases` does, rather than a disabled one |
| Ledger CLI | gate | `jails check` is 1.9 s here against 293 s for the Spring apps, which is the whole argument for §10.2's `test --fast`: the latency is Spring context startup and containers, not Maven | measure per-app rather than per-suite, and keep D as the floor |
| Harness | disk | ~~fixtures accumulate until `/tmp` fills and the suite fails in a way that looks like a code regression~~ **fixed**: `temp_dir` sweeps fixtures older than an hour, once per process, keeping this run's evidence and any concurrent run's | if it recurs, the next step is a `JAILS_KEEP_FIXTURES=1` opt-in and cleanup by default on success |
