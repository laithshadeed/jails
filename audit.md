<!--
audit.md — one review of `claude/branch-audit-sol-docs-uxgupy` against
`simplify-sol.md` and `jdl-sol.md`, dated 2026-08-29 at `5cec56b`.
...
`claude/branch-audit-sol-docs-uxgupy` at `5cec56b`, fifteen commits of
canonical-compiler work on top of `61413d7`. The authorities are
...
so the three-way merge exits 129 and 58 tests fail for that reason alone.
...
### A5.5 G4 instruments the kernel being deleted

`fault::POINTS` (`crates/jails-commit/src/fault.rs:79`) enumerates 22
failpoints in the legacy commit kernel, and `5cec56b` correctly closed the
gap between the advertised and tripped sets. `jails-workspace::execute` has
no failpoints at all. The convergence property `simplify-sol.md` trades
rollback for — "a crashed command may leave a temporarily mixed but
individually valid tree; the next identical generation repairs it
deterministically" — is asserted in prose and proved nowhere.

### A5.6 The one gate is not reproducible, and is red on current stable

There is no `rust-toolchain.toml`; `.github/workflows/verify-rewrite.yml:45`
uses `dtolnay/rust-toolchain@stable`, and `mise.toml` pins Java, Maven, mvnd
and Gradle but not rustc. On 1.94 the clippy step of `mise run
verify-rewrite` fails:

```
error: this boolean expression can be simplified
  --> crates/jails-java/src/java.rs:686:16
  = note: `-D clippy::nonminimal-bool` implied by `-D clippy::all`
```

Legacy code, one-line fix. The finding is that the gate the cutover plan
rests on can go red from a compiler release.
...
10. **A6.1** — write the module docs while the reasons are still recoverable.

`A5.5` (canonical crash convergence) and `A3.14` (typed artifact IR) are the
two largest remaining pieces of the design and are not on this list because
they are phases, not fixes.
