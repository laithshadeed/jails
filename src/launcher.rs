...
///
/// **It has to equal the project's own JUnit version**, and getting it wrong
/// does not fail to resolve -- it fails at *run* time with a `NoSuchMethodError`
/// wrapped in "the versions of JUnit jars on the classpath are not properly
/// aligned". That is what a pinned guess produced here on the first try.
///
