...
/// One editable source: a project that has a model, or had one, is refused by
/// name.
///
/// The three cases are a model this jails reads, a model it cannot read
/// (`ledger.toml`), and a model that is *gone* -- a reader who deleted
/// `.jails/` and kept the application. Only the last is silent otherwise, and
/// it is the one that costs the most: seeding afresh reads the generated tree
/// as the reader's and refuses on the first collision, several commands later.
pub(crate) fn refuse_if_modelled(root: &Path) -> Result<()> {
    ...
    if let Some(evidence) = generated_source(&root.join("src")) {
        let evidence = evidence.strip_prefix(root).unwrap_or(&evidence).display();
        return Err(Failure::Told(format!(
            "no model at `{existing}`; this project was generated by jails and its model is \
             gone.\n       fix: restore it from git (`git restore {existing}`). {evidence} was \
             rendered from the model, and seeding a new one beside it would read every generated \
...
/// A file under `src/` that carries the provenance header the compiler writes.
///
/// **This is what tells a deleted model from a project that never had one.**
/// `.jails/` holds the model and the lock; `src/` holds what they rendered.
/// Delete the first and the second is still there, and jails' old answer was
/// to derive a fresh seed -- which reads every generated file as the reader's
/// own and collides on the first regeneration. The header is on every managed
/// file (`JavaUnit::render` writes it, and the resources carry the same line
/// under their own comment syntax), so one is enough and the first found is
/// the one named.
///
/// Bounded by construction: it walks the one source tree it is given, and
/// stops at the first hit. The subject is that tree rather than the project,
/// because a project is more than this function needs to know.
fn generated_source(sources: &Path) -> Option<std::path::PathBuf> {
    const MARKER: &str = "Generated by jails from art_";
    ...
    while let Some(directory) = stack.pop() {
        let mut entries: Vec<_> = std::fs::read_dir(&directory)
            ...
            .collect();
        // Sorted, files before directories, subdirectories pushed in reverse:
        // the file this names is the same file on every run. A refusal that
        // names whichever entry the filesystem handed back first reads as a
        // different refusal each time somebody hits it.
        entries.sort();
