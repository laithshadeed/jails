...
"#,
    ...
    let field_renamed = jails_cmd(&root, None)
        .args([
            "entity", "field", "rename", "WorkItem", "title", "summary", "--column", "preserve",
        ])
        .output()
        .unwrap();
    assert!(
...
fn canonical_database_and_safe_field_evolution_are_one_exact_compiler_path() {
    ...
    let refused = jails_cmd(&root, None)
        .args(["entity", "field", "add", "Note", "status:string!"])
        .output()
        .unwrap();
...
fn canonical_field_evolution_preserves_hand_edits_and_lowers_explicit_sql_policies() {
    ...
    let preserved = jails_cmd(&root, None)
        .args([
            "entity", "field", "rename", "Note", "title", "headline", "--column", "preserve",
        ])
        .output()
        .unwrap();
    assert!(
...
fn canonical_field_evolution_refuses_campaigns_and_referenced_field_drop_atomically() {
    ...
    for (arguments, expected) in [
        (
            vec![
                "entity", "field", "rename", "Note", "title", "headline", "--column", "rolling",
            ],
...
fn canonical_composite_index_is_model_data_and_one_forward_migration() {
    ...
    let added = jails_cmd(&root, None)
        .args(["entity", "index", "add", "Note", "title, id desc"])
        .output()
        .unwrap();
    assert!(
        added.status.success(),
    ...
    for arguments in [
        vec!["entity", "index", "add", "Note", "title, id desc"],
        vec!["entity", "index", "add", "Note", "missing"],
        vec!["entity", "index", "add", "Note", "title nulls first"],
    ] {
        let before = snapshot_tree(&root);
        let refused = jails_cmd(&root, None).args(arguments).output().unwrap();
        assert!(!refused.status.success());
...
fn canonical_storage_preserve_removes_projections_and_revive_reuses_the_table() {
    ...
    for mutation in [
        ["entity", "field", "add", "Note", "body:string?"].as_slice(),
        ["entity", "index", "add", "Note", "title"].as_slice(),
    ] {
    ...
    let wrong = jails_cmd(&root, None)
        .args(["entity", "revive", "Note", "--table", "wrong"])
        .output()
        .unwrap();
    assert!(!wrong.status.success());
    ...
    let revived = jails_cmd(&root, None)
        .args(["entity", "revive", "Note", "--table", "notes"])
        .output()
        .unwrap();
    assert!(
