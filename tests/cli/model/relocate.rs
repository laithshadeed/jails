...
fn age(root: &Path) -> Vec<(String, String)> {
    ...
    // The file records a managed file's bytes as the text they are, and the
    // type reads either shape (`jails_contracts::bytes_field`); the digest's
    // preimage below is still the form `serde` derives.
