...
fn spring_subjects(label: &str) -> [Subject; 1] {
...
fn adopted_project_is_treated_the_same(flavour: Adopted) {
    ...
    for subject in &subjects {
        ...
        // `adopt` writes `jails.toml` and nothing else -- it is configuration,
        // not a transaction -- so `.jails/` need not exist yet when the reader
        // opts the project into the model straight afterwards.
        fs::create_dir_all(subject.root.join(".jails")).unwrap();
...
#[test]
fn the_ci_workflow_is_byte_identical() {
...
#[test]
fn the_container_build_is_byte_identical() {
    for wrapper in [false, true] {
...
#[test]
fn formatting_installs_identically() {
