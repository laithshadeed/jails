...
/**
 * HTTP for {@link Author}.
 *
 * <p>Speaks in {@link AuthorRequest} and {@link AuthorResponse} rather
...
public class AuthorController {
    ...
    public List<AuthorResponse> list() {
        return service.findAll().stream().map(AuthorResponse::from).toList();
    }

    /** 404 rather than an empty 200: "no such thing" and "here is nothing" differ. */
    @GetMapping("/{id}")
    public ResponseEntity<AuthorResponse> byId(@PathVariable Long id) {
    ...
    public ResponseEntity<AuthorResponse> create(@Valid @RequestBody AuthorRequest request) {
        Author created = service.create(request.toDomain());
