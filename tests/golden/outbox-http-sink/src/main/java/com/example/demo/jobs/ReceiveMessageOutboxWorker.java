...
public final class ReceiveMessageOutboxWorker {
    ...
    public ReceiveMessageOutboxWorker(
            JdbcReceiveMessageOutbox outbox,
            List<ReceiveMessageOutboxSink> sinks,
            @Value("${outbox.receive-message.batch-size:100}") int batchSize) {
        this.outbox = outbox;
    ...
    private void publish(JdbcReceiveMessageOutbox.Claimed claimed) {
        ...
        try {
            ...
            log.warn("ReceiveMessage outbox attempt {} failed at sink {}", claimed.attempt(), attempted, recorded);
        } catch (RuntimeException failure) {
            outbox.fail(claimed.id(), failure);
            log.warn("ReceiveMessage outbox attempt {} failed at sink {}", claimed.attempt(), attempted, failure);
        }
    }
}
