 ...
 *
 * {@snippet :
 * try (var server = Server.start(0, Map.of("/health", request -> Response.ok("{\"status\":\"up\"}")))) {
...
public final class Server implements AutoCloseable {
    ...
    private Server(HttpServer http, ExecutorService requests) {
        this.http = http;
     ...
     */
    public static Server start(int port, Map<String, Handler> routes) {
        try {
            ...
            http.start();
            return new Server(http, requests);
