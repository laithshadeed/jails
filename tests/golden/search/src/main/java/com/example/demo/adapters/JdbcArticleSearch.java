...
import com.example.demo.app.ArticleSearch;
...
/**
 * PostgreSQL full-text search against the generated {@code search_vector} column.
 *
 ...
 */
@Repository
public class JdbcArticleSearch implements ArticleSearch {
    ...
    public List<Article> matching(String query, int limit) {
        return jdbc.sql(SQL)
                ...
                .query((rows, rowNumber) -> new Article(rows.getObject("id", UUID.class), rows.getString("title"), rows.getString("body")))
                .list();
    }
}
