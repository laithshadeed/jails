 ...
 *
 * <p>{@code fixedDelayString} reads {@code jobs.reconcile.delay} rather
...
public class ReconcileJob {
    ...
    @Scheduled(fixedDelayString = "${jobs.reconcile.delay:PT1M}")
    public void run() {
        try {
            work();
        } catch (RuntimeException failure) {
            // Swallowed deliberately: rethrowing here cancels all future runs.
            log.error("ReconcileJob failed; the schedule continues", failure);
        }
    }

    /** The actual work. Package-private so a test can call it directly. */
    void work() {
        log.info("ReconcileJob ran");
    }
}
