// Generated by jails from art_cmp_durable_job_item_dispatcher_store. Clean hand edits survive regeneration.
...
public class JdbcItemDispatcherStore implements ItemDispatcherQueue {
    ...
    @Override
    @Transactional
    public void enqueue(UUID id, AddItemCommand.Input work) {
        Objects.requireNonNull(id, "id is required");
        Objects.requireNonNull(work, "work is required");
        int inserted = db.sql("""
                        insert into item_dispatcher_jobs (id, payload, state, attempts, max_attempts,
        ...
        if (inserted == 0 && !payloadOf(id).equals(Optional.of(work))) {
            throw new ItemDispatcherQueue.IdempotencyConflictException(id);
        }
    }
    ...
    private Optional<AddItemCommand.Input> payloadOf(UUID id) {
        return db.sql("select payload::text from item_dispatcher_jobs where id = :id")
                .param("id", id)
                .query(String.class)
                .optional()
                .map(payload -> Json.parse(payload, AddItemCommand.Input.class));
    }
    ...
    public Optional<Claimed> claim() {
        return db.sql("""
                        with candidate as (
                            select id from item_dispatcher_jobs
                ...
                .query((rows, rowNumber) -> new Claimed(
                        rows.getObject("id", UUID.class),
                        Json.parse(rows.getString("payload"), AddItemCommand.Input.class),
                        rows.getInt("attempts")))
                .optional();
    }
    ...
    public void succeed(UUID id) {
        db.sql("""
                        update item_dispatcher_jobs set state = 'SUCCEEDED', lease_until = null,
    ...
    public record Claimed(UUID id, AddItemCommand.Input work, int attempt) {}
}
