...
/**
 * HTTP for {@link Widget}.
 *
 * <p>Speaks in {@link WidgetRequest} and {@link WidgetResponse} rather
...
public class WidgetController {
    ...
    public List<WidgetResponse> list() {
        return service.findAll().stream().map(WidgetResponse::from).toList();
    }

    /** 404 rather than an empty 200: "no such thing" and "here is nothing" differ. */
    @GetMapping("/{id}")
    public ResponseEntity<WidgetResponse> byId(@PathVariable UUID id) {
    ...
    @PostMapping
    public ResponseEntity<WidgetResponse> create(@Valid @RequestBody WidgetRequest request) {
        Widget created = service.create(request.toDomain());
        return ResponseEntity.created(URI.create(PATH + "/" + created.id()))
                .body(WidgetResponse.from(created));
    }
