...
| ~~`add format`~~ | **Done**, and it is the one feature that edits both ends of the file: Spotless is not bundled with Gradle, so it needs `id 'com.diffplug.spotless' version '...'` inside `plugins {}` -- a block only legal near the top, which is the opposite end from where a marked block goes. A build with **no** `plugins {}` block refuses rather than growing one: writing that block means deciding where the top of somebody else's script is, and getting it wrong is a build that no longer evaluates at all. `spotlessCheck` is wired into `check` by the plugin itself, so jails does not repeat it. Verified against Gradle 9.6.1: `check` fails on an unformatted file, and `remove format` returns the build file byte-for-byte |
| `jails fmt` | The *transactional* half. `route::format` runs the formatter in a sandbox laid out from the projection, so the reformat is a reviewed diff committed in the same transaction -- and it drives that with Maven. Gradle in a throwaway tree needs its wrapper, its caches and a writable `build/`, which is a different bargain. It refuses by name and points at `./gradlew spotlessApply`, which is what the project is already configured for |
